...
Graph Graph::operator-(const Graph &other) const {
    ...
    for (auto const &current_vertex : other.vertices) {
        if (vertices.count(current_vertex) > 0) {
            for (auto const &current_edge : edges) {
                if (edges_diff.count(current_edge) &&
                (  (current_vertex == current_edge.getSourceVertex()) ||
                    (current_vertex == current_edge.getDestinationVertex()) )) {
